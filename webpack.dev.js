...
module.exports = merge(common.config,{
    ...
    devServer: {//配置webpack-dev-server开发服务器，如果现在修改和保存任意源文件，web 服务器就会自动重新加载编译后的代码
        //contentBase: path.resolve(__dirname, "dist"),//告诉服务器从哪里提供内容。只有在你想要提供静态文件(比如assets不需要被打包的资源)时才需要。默认使用当前工作目录(项目根目录)作为提供内容的目录
        publicPath: "/",//此路径下的打包文件可在浏览器中访问。这里的路径应该是相对于output.path
        compress: true,
